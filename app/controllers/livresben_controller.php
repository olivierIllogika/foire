  ...
  function pickup_scan()
  {
    ...
    if (!empty($this->params['data']))
    {
      ...
      if ($ret)
      {
        ...
        {
          $this->models['evlivre']->logEvent(102,$livre['id'],$_SESSION['etudiant']['id'],"d�consigne ({$_SESSION['etudiant']['prenom']})");
        } // en_consigne
      ...
      {
          $this->models['evlivre']->logEvent(151,0,$_SESSION['etudiant']['id'],"�chec [{$this->params['data']['scan_box']}] ({$_SESSION['etudiant']['prenom']})");
  ...
  function sale_scan()
  {
    ...
    if (!empty($this->params['data']))
    {
      ...
      if (array_key_exists($modePaiement, $this->mode_paiement) && $facture)
      {
        ...
        // d�fini le mode de paiement de la facture
      ...
      if (strlen($input) > 13)
      {
        ...
        if ($etudiant)
        {
          ...
          else
          {
            ...
            {
              // kick!
              $request_content = "<td class=\"rowCommand\" colspan=\"3\">Niveau de sécurité insuffisant</td>";
      ...
      if ($livre)
      {
        ...
        if ($ret)
        {
          ...
          // vente existe diff�rente de cette facture
          ...
          // re-scan du livre dans la m�me facture
          {
...
//        $facture['total'] = $this->params['data']['totalPrice'];
        ...
        // nouvelle facture si "mode de paiement" d�fini ou aucune "derni�re facture" trouv�e
...
//        $facture['total'] = $this->params['data']['totalPrice'];
      ...
      {
        $this->models['evlivre']->logEvent(251,$fid,$_SESSION['etudiant']['id'],"�chec [{$this->params['data']['scan_box']}] ({$_SESSION['etudiant']['prenom']})");
  ...
  function pseudo_recup()
  {
    ...
    if ($ret)
    {
      ...
      $this->models['evetudiant']->logEvent(304,$codebar,"r�cup: pseudo ($commis_n)");
...
//      unset($_SESSION['persistent']['etudiant']);
  ...
  function back_scan()
  {
    ...
    if ($input == 'passe')
    {
      $this->models['evetudiant']->logEvent(303,$commis,"r�cup: {$_SESSION['persistent']['etudiant']} saut� ($commis_n)");
      ...
      $request_content = "<td class=\"rowCommand\" colspan=\"5\">Prêt pour le suivant</td>";
    }//passe
    ...
    if ($input == 'suivant')
    {
      ...
      if ($ret)
      {
        ...
        if ($_SESSION['persistent']['suivant'] > 1)
        {
          ...
          $this->models['evetudiant']->logEvent(351,$commis,
              ...
              " � {$_SESSION['persistent']['etudiant']} ($commis_n)");
        ...
        elseif (count($ret) == 2)
        {
          ...
          $request_content = "<td class=\"rowCommand\" colspan=\"5\"><blink>!</blink> $perdus livre(s) à rendre; valeur taxée: $valeur$ (+$vendu$ = $total$)</td>";
        }
        elseif ($ret[0]['unsold'] == 0) // argent
        {
          ...
          // return info/warning about what's left to give back
          $request_content = "<td class=\"rowCommand\" colspan=\"5\"><blink>!</blink> Argent à rendre</td>";
        }
              ...
              $_SESSION['persistent']['remettre']['num_cheque'] < 0)
      {
        ...
        $request_content = "<td class=\"rowCommand\" colspan=\"5\">Entrer numéro chèque <blink>!!!</blink></td>";
      }
      else
      { // !ret
        unset($_SESSION['persistent']['perdus']);
    ...
    if (!empty($_SESSION['persistent']['etudiant']) && ($input == 'argent' || $input == 'cheque' || $input == 'imp_cheque') )
    {
      ...
      $ret = $this->models['livre']->findBySql("SELECT l.id, titre, prix FROM livres AS l LEFT JOIN facture_lignes AS fl ON fl.livre_id=l.id JOIN factures AS f ON f.id=fl.parent_id WHERE f.created > DATE_SUB(NOW(),INTERVAL 30 DAY) AND en_consigne=1 AND NOT ISNULL(livre_id) AND codebar={$_SESSION['persistent']['etudiant']} ");

      if (!$ret)
      {
        if ($input == 'cheque' && !empty($_SESSION['persistent']['remettre']['argent_back']) )
        {
          $request_content = "<td class=\"rowCommand\" colspan=\"5\">Entrer numéro de chèque corrigé <blink>!</blink></td>";
          $_SESSION['persistent']['remettre']['num_cheque'] = -2;
        }
        elseif ($input == 'imp_cheque' && !empty($_SESSION['persistent']['remettre']['argent_back']) )
        {
          $request_content = "<td class=\"rowCommand\" colspan=\"5\">Réimpression... Entrer nouveau numéro de chèque <blink>!</blink></td><!-- imp_cheque -->";
      ...
      else
      {
        ...
        if ($input == 'imp_cheque')
        {
          $request_content = "<td class=\"rowCommand\" colspan=\"5\">Impression chèque...  entrer numéro <blink>!</blink></td><!-- imp_cheque -->";
        }
        elseif ($input == 'cheque')
        {
          $request_content = "<td class=\"rowCommand\" colspan=\"5\">Chèque manuel... entrer numéro <blink>!</blink></td>";
        }
        else
        {
          $_SESSION['persistent']['remettre']['num_cheque'] = 0;
    ...
    if ($input)
    {
          ...
          $_SESSION['persistent']['remettre']['num_cheque'] < 0 )
      {
        ...
        {
          $this->models['evetudiant']->logEvent(305,$commis,"r�cup: cheque # $input {$_SESSION['persistent']['remettre']['argent_back']}$ � {$_SESSION['persistent']['etudiant_nom']} ({$_SESSION['persistent']['etudiant']}) de ($commis_n)");
        }
        else if ($_SESSION['persistent']['remettre']['num_cheque'] == -2)
        {
          $this->models['evetudiant']->logEvent(306,$commis,"r�cup: cheque (correction) # $input {$_SESSION['persistent']['remettre']['argent_back']}$ � {$_SESSION['persistent']['etudiant_nom']} ({$_SESSION['persistent']['etudiant']}) de ($commis_n)");
        }
        else if ($_SESSION['persistent']['remettre']['num_cheque'] == -3)
        {
          $this->models['evetudiant']->logEvent(307,$commis,"r�cup: r�impression cheque # $input {$_SESSION['persistent']['remettre']['argent_back']}$ � {$_SESSION['persistent']['etudiant_nom']} ({$_SESSION['persistent']['etudiant']}) de ($commis_n)");
        }
        
        $_SESSION['persistent']['remettre']['num_cheque'] = $input;
        $request_content = "<td class=\"rowCommand\" colspan=\"5\">Chèque #$input</td>";
      }
      ...
      // aucun etudiant au dossier ouvert
      {
        ...
        {
          // demande la page avec les infos de l'�tudiant
          $_SESSION['persistent']['etudiant'] = $etudiant['id'];
          $_SESSION['persistent']['etudiant_nom'] = $etudiant['prenom'].' '.$etudiant['nom'];
          
          $this->models['evetudiant']->logEvent(301,$commis,"r�cup: affichage du dossier *$input* ($commis_n)");
...
/**************************************************************************/
          ...
          $this->models['evlivre']->logEvent(304,$livre['id'],$commis,"r�cup: retour au consignataire ($commis_n)");
  ...
  function etiquettes($codebar=null)
  {
    ...
    if (!$ret)
    {
      ...
      if (!$ret)
      {
        ...
        $this->models['evlivre']->logEvent(451,0,$codebar,"�tiquette; non-inscrit");
      }
      elseif ($ret[0]['confirme'] == 0)
      {
        // pas confirm�
        $this->models['livre']->validationErrors['confirmation'] = 1;
        $this->models['evlivre']->logEvent(452,0,$codebar,"�tiquette; non-confirm�");
      }
      else
      {
        // pas d'�tiquettes
...
//echo $_SERVER['SCRIPT_NAME'];
  ...
  function recu($methode=null)
  {
    ...
    if (!$methode)
    {
      ...
      $this->models['evlivre']->logEvent(152,0,$_SESSION['etudiant']['id'],"m�thode de recu nulle ({$_SESSION['etudiant']['prenom']})");
    ...
    if (empty($_SESSION['persistent']['dernier_codebar']))
    {
      ...
      $this->models['evlivre']->logEvent(153,0,$_SESSION['etudiant']['id'],"aucun �tudiant d�fini pour le recu ({$_SESSION['etudiant']['prenom']})");
      $this->render('cueillette');
      return;
    }

    $this->set('recuperation', $this->models['journee']->find("activite = 'Récupération' AND session = {$_SESSION['foire']['session']}"));
    ...
    if (!$ret)
    {
      ...
      $this->models['evlivre']->logEvent(154,0,$_SESSION['etudiant']['id'],"aucun livres � imprimer sur le re�u de $codebar ({$_SESSION['etudiant']['prenom']})");
      $this->render('cueillette');
      return;
    }
...
print $_SERVER['SCRIPT_FILENAME'];
    ...
    if ($methode == 'imprimer')
    {
      $this->models['evlivre']->logEvent(103,0,$_SESSION['etudiant']['id'],"impression du re�u pour $codebar ({$_SESSION['etudiant']['prenom']})");
      $this->set('redirect', "{$this->base}/livresben/recu_flash");
      $this->render('recu_pdf','back_download');
    }
    elseif ($methode == 'courriel')
    {
      $this->models['evlivre']->logEvent(104,0,$_SESSION['etudiant']['id'],"re�u par courriel pour $codebar ({$_SESSION['etudiant']['prenom']})");
    ...
    else
    {
      ...
      $this->models['evlivre']->logEvent(155,0,$_SESSION['etudiant']['id'],"m�thode de recu inconnue ({$_SESSION['etudiant']['prenom']})");
      $this->render('cueillette');
    }
  }
